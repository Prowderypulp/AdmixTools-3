...
pub fn run_qpfstats(
    ...
    let num_blocks = (block_assignments.iter().max().cloned().unwrap_or(0) + 1) as usize;
    if num_blocks <= 1 {
    ...
    for (snp_i, snp) in snps.iter().enumerate() {
        before_setwt_snps += 1;
        ...
        let block_idx = block_assignments[snp_i];
        ...
        if !mask[anchor_idx] { continue; }
        let all_present = mask.iter().all(|&m| m);
    ...
    for k in 0..num_blocks {
        for idx in 0..num_pops * num_pops {
            ...
            // count is handled by wjack for each block
        }
    }
    ...
    let total_wjack: f64 = wjack.iter().sum();

    for i in 0..num_pops {
        for j in i + 1..num_pops {
            let idx = i * num_pops + j;
            let mean_en = g_pair_top[idx] / (total_wjack + 1e-10);
            ...
            for &k in &active_blocks {
                // In fstmode == NO, C jackknifes the numerator mean
                let tk = g_pair_top[idx] - pair_btop[k][idx];
                let bk = total_wjack - wjack[k] + 1e-10;
                jmeans.push(tk / bk);
            }
    ...
    // Flatten: gbot observations + N(0,1) prior — stabilizes tiny samples.
    for j in 0..nfstats {
        let y1 = gbot[j] + 1.0;
