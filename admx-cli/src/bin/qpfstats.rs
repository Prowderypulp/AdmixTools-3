...
fn main() -> AdmxResult<()> {
    ...
    // n = size of the full canonical f-statistic enumeration (all f2 + anchored
    // f3 + canonical f4), matching C qpfstats.c. Not np*(np+1)/2.
    let n = FBasis::full_stats(np).len();
    ...
    for i in 0..nbasis {
        ...
        if let FStatKind::F3(_, a, b) = kind {
            ...
            // C qpfstats.c:691-696 — fitted basis mean/sigma, then the raw
            // per-fstat mean/sigma after `::`.
            println!("basis: {:3} {:3} {:12.6} {:12.6} :: {:12.6} {:12.6}",
                     i1, i2, means[i], covar[[i, i]].sqrt(), result.fs_mean[i], result.fs_sig[i]);
        }
    }
