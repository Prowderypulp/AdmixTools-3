...
//! Ported from `convertf-rs/src/geno/codec.rs`.
...
///
/// `dst.len()` must be `>= ceil(src.len() * 2 / 8)`. Trailing bits are zero-padded.
pub fn pack(src: &[u8], dst: &mut [u8]) {
    let need = (src.len() * 2 + 7) / 8;
    debug_assert!(dst.len() >= need);
    for b in dst[..need].iter_mut() { *b = 0; }
...
pub fn unpack(src: &[u8], n: usize, dst: &mut [u8]) {
    ...
    for i in 0..n {
        dst[i] = unpack_genotype(src, i);
    }
}

/// Unpack a single genotype from canonical 2-bit MSB-first.
#[inline(always)]
pub fn unpack_genotype(src: &[u8], i: usize) -> u8 {
    let byte = i / 4;
    let shift = 6 - 2 * (i % 4);
