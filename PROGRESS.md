...
See `../RUST_PORT_PHASE2.md`. Quick state of the stubs that roll over:

- `admx-rank` — stub. `ranktest`, `ranktestfix`, `normab`, `checkmv` all doc-only.
- `admx-qpwave` — stub. Driver is one line of doc.
- `admx-qpadm` — stub. `prng.rs` has ~43 lines of partial ranmar; rest is doc-only.
- `admx-cli/qpWave`, `admx-cli/qpAdm` — `println!("stub")` bins.
